...
export default function FiltersDialog({ isOpen, setIsOpen }: Readonly<FiltersDialogProps>) {
  ...
  const artsTranslations = useTranslations("Arts.filtering");
  const form = useForm({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{artsTranslations("title")}</DialogTitle>
          <DialogDescription>{artsTranslations("description")}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form className={"flex flex-col gap-2"} onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{artsTranslations("minimumRating")}</FormLabel>
                  <SelectComponent
                    ...
                    placeholder={artsTranslations("selectRating")}
                  />
                </FormItem>
              )}
            />
            ...
            <div className={"flex gap-2 justify-between w-full"}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{artsTranslations("yearSince")}</FormLabel>
                    <SelectComponent
                      ...
                      value={field.value ?? ""}
                      defaultValue={field.value}
                      placeholder={artsTranslations("selectYear")}
                    />
                  </FormItem>
                )}
              />
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{artsTranslations("yearUntil")}</FormLabel>
                    <SelectComponent
            ...
            <DialogFooter className={"mt-4"}>
              <Button type="submit">{artsTranslations("applyFilters")}</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
