...
import { CountryQueries } from "@/lib/queries/country.queries";
import { GenreQueries } from "@/lib/queries/genre.queries";
import { StatusQueries } from "@/lib/queries/status.queries";
import { TypeQueries } from "@/lib/queries/type.queries";
import artsRequests from "@/src/http-requests/arts-requests";
import { Country } from "@/src/types/models/country.types";
...
export const CreateArtView = () => {
  const { data: countries = [] } = useQuery(CountryQueries.getCountries());
  const { data: types = [] } = useQuery(TypeQueries.getTypes());
  const { data: statuses = [] } = useQuery(StatusQueries.getStatuses());
  const { data: genres = [] } = useQuery(GenreQueries.getGenres());
  ...
  const createArtMutation = useMutation({
    mutationFn: async (data: FormData) => {
      try {
        return await artsRequests.createArt(data);
      } catch {
    ...
    onError: () => {
      toast({
        title: "An error occurred while creating the art",
        variant: "destructive",
      });
    },
  });
      ...
      <Form {...form}>
        <form className={"w-full max-w-[400px] flex flex-col gap-4"} onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormField
            ...
            render={({ field }) => (
              <SelectComponent
                options={statuses.map((item: Type) => ({
                  label: item.name,
                  value: item.id,
                }))}
                value={field.value?.toString()}
