...
import authRequests from "@/src/http-requests/auth-requests";
...
const SignUpSchema = z
  .object({
    password: z.string().min(3, "Пароль должен состоять минимум из 3-х символов"),
    passwordRepeat: z.string().min(3, "Пароль должен состоять минимум из 3-х символов"),
    displayName: z.string().min(1, "Поле обязательно к заполнению"),
    email: z
      .string()
      .min(1, "Поле обязательно к заполнению")
      .email("Вы ввели невалидную почту. Пример валидной почты: test@gmail.com"),
  })
...
const SignUpView = () => {
  ...
  const onSubmit = async (data: z.infer<typeof SignUpSchema>) => {
    try {
      setIsPending(true);
      await authRequests.registration(data);
      signIn("credentials", {
        redirect: false,
        username: data.email,
